...
figure=plt.figure()
plt.plot(baseline_df['dead'],label='real')
for i in range(4):
    data_path=proj_path+r'/data/result/online working/'+f'online{i}.csv'
    data_df=pd.read_csv(data_path,
                        index_col='date',
                        date_parser=lambda x: datetime.strptime(x, '%Y-%m-%d'))
    plt.plot(data_df['dead'], label=f'{(i+1)*20}% from 31, May')
for i in range(4):
    data_path=proj_path+r'/data/result/online working/'+f'online late{i}.csv'
    data_df=pd.read_csv(data_path,
                        index_col='date',
                        date_parser=lambda x: datetime.strptime(x, '%Y-%m-%d'))
    plt.plot(data_df['dead'], label=f'late{i}')
plt.legend()
plt.xlabel('Date')
plt.ylabel('Mortality')
plt.title("Mortality on Different Online Working's rate")
